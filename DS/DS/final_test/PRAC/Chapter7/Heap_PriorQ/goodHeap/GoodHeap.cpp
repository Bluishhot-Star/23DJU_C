...
void HInsert(Heap* H, Hdata data)
{
    ...
    while (idx != 1)
    {
        if ( H->comp(data, H->heapArr[GetParentIDX(idx)]) > 0) // data의 우선순위가 높다면
        {
...
{
    Hdata temp = H->heapArr[1];
    Hdata lastElem = H->heapArr[H->numOfData];
    ...
    {
        if (H->comp(lastElem, H->heapArr[childIdx]) >= 0) //반복 종료 조건 2. 자식의 우선순위가 더 높음 (최소 힙)
            break;
