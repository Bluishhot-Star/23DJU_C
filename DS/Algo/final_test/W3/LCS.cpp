...
int LCS(char* X, char* Y, int i, int j, LCSTable* T){
    ...
    for ( m = 0; m <= i; m++) T->data[m][0] = 0;
    for ( n = 0; n <= j; n++) T->data[0][n] = 0;
    
    for ( m = 1; m <= i; m++)
    {
        for ( n = 1; n <= j; n++)
        {
            if (X[m-1] == Y[n-1]) T->data[m][n] = T->data[m - 1][n - 1] + 1;
            else
            {
                if(T->data[m][n-1] >= T->data[m-1][n]) T->data[m][n] = T->data[m][n-1];
                else T->data[m][n] = T->data[m-1][n];
            }
...
int main(void){
    ...
    for ( i = 0; i < LX + 1; i++)
    {
        ...
        memset(T.data[i], 0, sizeof(int) * (LX + 1));
    }
